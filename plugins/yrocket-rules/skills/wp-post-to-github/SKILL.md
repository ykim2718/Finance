...
Rev. 13 | Created: 2026-08-20 | Updated: 2026-08-25 06:07 UTC
...
보여준다. 값은 사용자의 환경에 맞게 두고, `<...>` 자리만 채워 쓰라고 안내한다.
...
| `WP_USERNAME` | `ykim2718@gmail.com` |
...
인증은 환경변수로 읽는다. 값을 대화에 다시 적지 않는다. 등록하는 자리는 2 를 본다.

| Variable | Example | Note |
|----------|---------|------|
| `WP_URL` | `https://ykim.synology.me/wordpress` | WordPress 설치의 주소. |
| `WP_USERNAME` | `ykim2718@gmail.com` | application password 를 발급할 때 붙인 **이름이 아니라 로그인 계정**이다. |
| `WP_APP_PASSWORD` | `xxxx xxxx xxxx xxxx xxxx xxxx` | WordPress application password 이며 로그인 비밀번호가 아니다. |

환경변수가 없으면 사용자에게 요청한다. `WP_USERNAME` 에 이름을 넣으면 `invalid_username` 이
돌아온다. 계정을 모르면 email 을 먼저 시도한다.
