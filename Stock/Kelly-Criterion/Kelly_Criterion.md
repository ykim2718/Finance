...
<p>Fig 1. Expected log growth against bet size, with the simulated medians overlaid</p>
...
<p>Fig 2. Simulated growth distribution by bet size</p>
...
<p>Fig 3. Growth curve split into four zones at half, one and two times the optimum</p>
...
## Appendix B. Growth from a flat price

본문의 게임은 배수가 2배와 반토막이라 값이 크다. 이 부록은 훨씬 얌전한 가격 경로 하나를 끝까지 따라가, 제자리걸음 하는 주가에서도 비중 선택만으로 성장이 나오는 것을 보인다. 주가는 1 에서 출발해 홀수 날 10% 내리고 짝수 날 $1/9$ 만큼 올라 정확히 1 로 돌아오며, 이를 100일 반복한다. 투자금은 100 이고, 매일 자산의 고정 비율 $f$ 를 그 주식에 두고 나머지는 현금으로 둔다. 현금 배수는 본문과 같이 $c = 1$ 이다.

<img src="Kelly_Criterion_fig/alternating/wealth_paths.png" width="900" style="max-width: 100%; display: block;" alt="Fig 4">
<p>Fig 4. Flat price path and the wealth it produces at five bet fractions</p>
...
주가는 100일 뒤 정확히 1 로 돌아오는데 가운데 세 비중은 자산을 늘렸다. 이틀이 한 주기이므로 100일은 주기 50회이고, 한 주기의 배수는 다음과 같다.

$$(1 - 0.10 f)\left(1 + \frac{f}{9}\right) = 1 + \frac{f (1 - f)}{90}$$

$f (1 - f)$ 가 $f = 0$ 과 $f = 1$ 에서 0 이고 $f = 0.5$ 에서 최대이므로, 100일 뒤 자산은 곧 다음 값이다.

$$W_{100} = 100 \left(1 + \frac{f (1 - f)}{90}\right)^{50}$$

$f = 0.5$ 를 넣으면 $100 \times (1 + 1/360)^{50} = 114.8775$ 로 Table 2 의 가운데 행과 같다.

양 끝이 100 인 이유는 서로 다르다. $f = 0$ 은 전액을 현금으로 두므로 주가와 무관하게 움직이지 않는다. $f = 1.00$ 은 투자금 전액을 그 주식에 두는 것이어서 현금 몫이 0 이 되고, 자산 경로가 주가 경로에 100 을 곱한 것과 같아진다. Fig 4 의 (a) 가 1 에서 끝나므로 (b) 의 $f = 1.00$ 선도 100 에서 끝난다.

가운데가 뜨는 것은 매일 비중을 $f$ 로 되돌리는 행위 자체가 만드는 이득이다. 오른 날에는 늘어난 주식을 덜어내고 내린 날에는 싸진 주식을 채우므로, 주가가 제자리라도 자산은 올라간다. 2.3 절의 식이 이 값을 정확히 짚는다. $u = 1 + 1/9$, $d = 0.9$, $c = 1$, $p = 0.5$ 를 넣으면 분자의 $p A + (1-p) B$ 가 $0.5 \times (1/9) - 0.5 \times 0.10 = 1/180$ 이고 $A B = -1/90$ 이므로 $f^{\ast} = 0.5$ 이다. Table 2 의 최대값이 바로 그 비중에 있다.

4.1 절의 zone 경계도 이 표에서 그대로 읽힌다. $2 f^{\ast} = 1.00$ 이 성장률이 0 으로 되돌아오는 지점이고, Table 2 의 마지막 행이 정확히 100 인 것이 그 값이다. $f = 0.25$ 와 $f = 0.75$ 가 같은 110.9665 로 끝나는 것도 우연이 아니다. $f (1-f)$ 는 $f^{\ast}$ 를 축으로 대칭이므로 최적점에서 같은 거리만큼 떨어진 두 비중은 같은 성장률을 갖는다. 3.3 절이 말하는 대칭이 이것이다.

이 경로에서 1.00 이 비중의 상한은 아니다. 하루 최대 손실이 10% 이므로 2.4 절의 파산 비중은 $1 / 0.10 = 10$ 이고, 그 아래라면 빌린 돈을 더해 1.00 을 넘길 수 있다. 다만 $f \gt 1$ 에서는 $f (1-f)$ 가 음수이므로 자산이 줄어든다.
...
| `--up-rate` | float | 1/9 | no | 상승일의 수익률 |
| `--down-rate` | float | 0.10 | no | 하락일의 손실률 |
| `--first-day` | `up` or `down` | `down` | no | 주기의 첫 날 |
...
기본값의 두 비율은 서로를 정확히 되돌리므로 주가가 제자리를 지킨다. 두 비율을 따로 주면 주가가 추세를 갖게 되고, 그때도 위의 두 식은 그대로 성립한다. 음수이거나 파산 비중 이상인 `--fractions` 는 에러로 막는다. 그 비중에서는 하락일 하루가 자산을 0 이하로 만들어 이후의 경로가 뜻을 잃기 때문이다.
...
<p>Fig 5. Realised growth against bet size, with the curve the full-sample estimate predicts</p>
...
<p>Fig 6. Estimated Kelly fraction by window and the distribution of out-of-sample growth</p>
